...
| File | Action |
|---|---|
| `app/composables/themes-data.ts` | Full rewrite — all 4 blocks expanded |
| `app/composables/useThemes.ts` | No change |
| `app/components/EditorSidebar.client.vue` | No change |
| `app/composables/useBlockRegistry.ts` | No change |

---

### Task 1: Navbar block — interface, defaults, fields, render

**Files:**
- Modify: `app/composables/themes-data.ts`

- [ ] **Step 1: Replace the NavLink interface and entire Navbar block section**

Find and replace everything from `// ─── Navbar block editor data` through the closing `}` of `renderRu1Navbar` with:
...
```bash
git add app/composables/themes-data.ts
git commit -m "feat: expand Navbar block — logo, sticky, search placeholder, sign-in/cart toggles, text/border/font controls"
```

---

### Task 2: Hero block — interface, defaults, fields, render

**Files:**
- Modify: `app/composables/themes-data.ts`

- [ ] **Step 1: Replace the entire Hero block section**

Find and replace everything from `// ─── Hero block editor data` through the closing `}` of `renderRu1Hero` with:
...
```bash
git add app/composables/themes-data.ts
git commit -m "feat: expand Hero block — headline/subheadline overlay, CTA button, overlay opacity, text alignment"
```

---

### Task 3: Featured Products block — interface, defaults, fields, render

**Files:**
- Modify: `app/composables/themes-data.ts`

- [ ] **Step 1: Replace the entire Featured Products block section**

Find and replace everything from `// ─── Featured Products block editor data` through the closing `}` of `renderRu1Products` with:
...
```bash
git add app/composables/themes-data.ts
git commit -m "feat: expand Featured Products block — section bg/padding, card radius, button colours, old price, removed hardcoded swatches"
```

---

### Task 4: Footer block — interface, defaults, fields, render

**Files:**
- Modify: `app/composables/themes-data.ts`

- [ ] **Step 1: Replace the entire Footer block section**

Find and replace everything from `// ─── Footer block editor data` through the closing `}` of `renderRu1Footer` with:
...
```bash
git add app/composables/themes-data.ts
git commit -m "feat: expand Footer block — padding and border style/width/color controls"
```
...
pnpm typecheck
```
Expected: no errors. If TypeScript complains about `ProductSwatch` being referenced, delete the `export interface ProductSwatch` definition (it was removed in Task 3).

- [ ] **Step 4: Commit**

```bash
git add app/composables/themes-data.ts
git commit -m "refactor: derive theme section html_code from render(defaults) so initial HTML always matches editor defaults"
```
...
**Spec coverage:**
- ✅ Navbar: logo, bgColor, sticky, searchPlaceholder, navLinks with visible toggle, showSignIn/showContactUs/showCart with label+url, textColor, fontSize, fontWeight, paddingY/X, logoWidth, border* — Task 1
- ✅ Hero: headline/subheadline overlay, textColor, textAlign, overlayColor/Opacity, ctaText/Url/BgColor/TextColor, bgColor, paddingY/X, borderRadius, aspect-ratio 4:1 fixed in render — Task 2
- ✅ Featured Products: bgColor, paddingY/X, cardBorderRadius, buttonBgColor/TextColor, oldPriceColor, oldPrice per product — Task 3
- ✅ Footer: paddingY/X, borderStyle/Width/Color — Task 4
- ✅ Section html_code always matches defaults — Task 5
- ✅ All fallback rules: empty logoUrl → text, invisible navLinks omitted, empty headline → no text layer, empty oldPrice → no `<s>`, borderStyle none → no border — implemented in render functions

**Placeholder scan:** No TBDs. All code is complete.

**Type consistency:** `NavLink.visible: boolean` added in Task 1 and used in render filter. `Product` drops `swatches` in Task 3 consistently across interface, defaults, and render. `_colClass` kept in Task 3 (not duplicated from old file).
